...
class CommentController {
  ...
  async store ({ auth, request, response }) {
    const newCommentDataRequest = request.only(['thread_id','content'])
    const newCommentData = Object.assign({}, {...newCommentDataRequest, user_id: auth.user.id})

    const newComment = await Comment.create(newCommentData)

    response.redirect(`/threads/${newComment.thread_id}`)
  }
